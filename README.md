...
SUPPORT_PROMPT_MANAGED_SERVER_TEXT=Enjoying Tweeticcini? Vote on top.gg! It would make Mocha's day 💛
...
- set `SUPPORT_PROMPT_MANAGED_SERVER_TEXT` to customize the top.gg vote prompt only for servers listed in `SUPPORT_PROMPT_SERVER_IDS`; other eligible servers keep the default prompt
...
- if `free_tweets_check_period` is omitted, Tweeticcini falls back to `120` seconds for Free servers
...
Current public pricing expects `STRIPE_PRICE_ID_PLUS` to point at the recurring Plus $3.99/month price and `STRIPE_PRICE_ID_PRO` to point at the recurring Premium $6.99/month price. The app still uses the internal `pro` plan key for Premium.
